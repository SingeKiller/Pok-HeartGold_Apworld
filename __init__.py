...
# (true in a real Archipelago load, since that's what's importing it) and
# that `data/` already exists (bundled into the built `.apworld`, or
# generated by whatever test fixture imports this file -- see
# tests/test_world_init.py's own `data_gen.py` subprocess call before it
# spec-loads this module). An earlier revision tried to paper over both
# with a dev-only bootstrap here; that broke loading from a real, zipimport-
# ed `.apworld` (`os.path.isdir()` on a path inside a zip is always False,
# so the fallback fired unconditionally -- see M3's R2 review) and had no
# effect on pytest either (`--import-mode=importlib`, already set in
# pyproject.toml, never imports this file during collection). Removed.
